#!/usr/bin/env node
// Deterministic task-checklist builder for the app-orchestrator-kit.
...
// Screen-less stories: any story not covered by a screen-based task above.
const coveredStoryIds = new Set(tasks.flatMap((t) => t['story-refs']))
for (const story of stories) {
  if (coveredStoryIds.has(story.id) || story.priority === 'wont') continue
  const acIds = acs.filter((ac) => ac['story-ref'] === story.id).map((ac) => ac.id)
  const prior = existingByScreen.get(story.id)
  tasks.push({
    id: allocateId(prior?.id),
    title: `${story.as}: ${story['i-want']}`,
    'screen-ref': '',
    'story-refs': [story.id],
    'ac-refs': acIds,
    'entity-refs': [],
    priority: story.priority,
    'slug-hint': kebab(`${story.as} ${story['i-want']}`) || kebab(story.id),
    status: prior?.status ?? 'pending',
    slug: prior?.slug ?? '',
    branch: prior?.branch ?? '',
    'blocked-reason': prior?.['blocked-reason'] ?? '',
...
// Prior tasks whose source screen/story no longer exists in the spec → blocked, never dropped.
const currentRefs = new Set(tasks.map((t) => t['screen-ref'] || t['story-refs'][0]))
for (const [ref, prior] of existingByScreen) {
  ...
  tasks.push({
    ...prior,
    status: 'blocked',
    'blocked-reason': 'source screen or story removed from spec',
  })
}
...
if (tasks.length === 0) {
  console.error(`No buildable tasks derived from ${specPath} (no non-"wont" user-stories/screens).`)
  process.exit(1)
}
