...
> kits and the checklist loop around them.
>
...
| 1 | Spec | `spec-dev-kit:generate-spec` | Invoke only if no `status: approved` spec. Requires `.spec/context/*.md`. Capture `{RUN_DIR}/kit-result.json` |
| 2 | Prototype | `html-generator-kit:generate-html` | Optional. Pass `SPEC_PATH` (file path). Capture `{dirname(SPEC_PATH)}/html-kit-result.json` |
| 2a | Checklist derivation | `scripts/build-checklist.mjs` + this skill | Script reads `spec.md` from disk. Human confirms the list |
...
| 4 | Report | this skill *(inline)* | Paths + checklist counts; remind `/create-pr` |
