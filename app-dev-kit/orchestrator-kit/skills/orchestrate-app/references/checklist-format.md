...
**Written by**: `scripts/build-checklist.mjs` (Station 2a, create) and the `orchestrate-app` skill
(Station 3, status updates)
**Read by**: `orchestrate-app` skill, on every invocation (Station 0 resume check)
...
`build-checklist.mjs` enforces these on write; the orchestrate-app skill re-checks them on read
