...
spec-dev-kit describes a whole surface, so this kit must split it into independently buildable
**UI screen-tasks** before the task loop (Station 3) starts.
...
Screens with **no** matching story (e.g. a shared shell/settings page nobody wrote a story for)
still become a task — the checklist confirmation gate (Station 2a) is where a human drops ones
that aren't worth building.
